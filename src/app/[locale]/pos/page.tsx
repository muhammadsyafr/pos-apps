        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4 lg:mb-6">
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <div className="flex items-center bg-white dark:bg-slate-800 rounded-full px-3 lg:px-4 py-2 w-full sm:w-48 lg:w-64 border border-slate-200 dark:border-slate-700">
              ...
              <input className="bg-transparent border-none focus:ring-0 text-sm w-full text-slate-900 dark:text-slate-50 placeholder:text-slate-400 dark:placeholder:text-slate-500" placeholder={t("searchProducts")} value={search} onChange={e => setSearch(e.target.value)} />
            </div>
            <Select value={selectedCategory} onValueChange={setSelectedCategory}>
              <SelectTrigger className="w-full sm:w-40 bg-white dark:bg-slate-800 rounded-full border border-slate-200 dark:border-slate-700">
        ...
        <div className="flex-1 overflow-auto">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-3 lg:g-4">
            {filteredProducts.map(product => (
              <button key={product.id} onClick={() => addToCart(product)} disabled={product.stock === 0} className="bg-white dark:bg-slate-800 rounded-xl lg:rounded-2xl p-3 lg:p-4 text-left hover:shadow-lg hover:shadow-blue-900/5 dark:hover:shadow-blue-400/10 transition-all disabled:opacity-50 group">
                <div className="aspect-square bg-slate-100 dark:bg-slate-700 rounded-lg lg:rounded-xl mb-2 lg:mb-3 flex items-center justify-center overflow-hidden">
                  {product.imageUrl ? <img src={product.imageUrl} alt={product.name} className="w-full h-full object-cover" /> : <svg className="w-8 lg:w-12 h-8 lg:h-12 text-slate-300 dark:text-slate-500" fill="currentColor" viewBox="0 0 24 24"><path d="M18.5 3H6c-1.1 0-2 .9-2 2v5.71c0 3.83 2.95 7.18 6.78 7.29 3.96.12 7.22-3.06 7.22-7v-1h.5c1.93 0 3.5-1.57 3.5-3.5S20.43 3 18.5 3z" /></svg>}
                </div>
                <h3 className="font-headline font-semibold text-sm text-slate-900 dark:text-slate-50 truncate">{product.name}</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400 mb-1 lg:mb-2">{product.sku}</p>
                <div className="flex items-center justify-between">
                  <span className="font-headline font-black text-blue-700 dark:text-blue-400 text-base lg:text-lg">{formatIDR(product.sellPrice)}</span>
      ...
      <div className="fixed bottom-0 left-0 right-0 lg:bottom-auto lg:top-16 lg:left-auto lg:right-0 lg:w-96 lg:min-w-96 lg:h-[calc(100vh-4rem)] bg-slate-100 dark:bg-slate-900 flex flex-col border-t lg:border-t-0 lg:border-l border-slate-200 dark:border-slate-700 shadow-xl lg:shadow-none z-20">
        ...
        <div className="flex-1 overflow-auto p-4 lg:p-6 space-y-3">
          {cart.length === 0 ? (
            ...
            cart.map(item => (
              <div key={item.id} className="flex gap-2 lg:gap-3 p-2 lg:p-3 bg-white dark:bg-slate-800 rounded-lg">
                <div className="w-12 lg:w-14 h-12 lg:h-14 bg-slate-100 dark:bg-slate-700 rounded-lg flex-shrink-0 flex items-center justify-center">
                  {item.imageUrl ? <img src={item.imageUrl} alt={item.name} className="w-full h-full object-cover rounded" /> : <svg className="w-5 lg:w-6 h-5 lg:h-6 text-slate-300 dark:text-slate-500" fill="currentColor" viewBox="0 0 24 24"><path d="M18.5 3H6c-1.1 0-2 .9-2 2v5.71c0 3.83 2.95 7.18 6.78 7.29 3.96.12 7.22-3.06 7.22-7v-1h.5c1.93 0 3.5-1.57 3.5-3.5S20.43 3 18.5 3z" /></svg>}
                </div>
                <div className="flex-1 min-w-0">
                  <h4 className="font-headline font-semibold text-sm truncate text-slate-900 dark:text-slate-50">{item.name}</h4>
                  <p className="text-xs text-slate-500 dark:text-slate-400">{formatIDR(item.sellPrice)}</p>
                </div>
                <div className="flex items-center gap-1">
                  <button onClick={() => updateQuantity(item.id, -1)} className="w-6 lg:w-7 h-6 lg:h-7 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 flex items-center justify-center hover:bg-slate-200 dark:hover:bg-slate-600 text-sm font-bold">-</button>
                  <span className="w-6 lg:w-8 text-center font-headline font-bold text-sm text-slate-900 dark:text-slate-50">{item.quantity}</span>
                  <button onClick={() => updateQuantity(item.id, 1)} className="w-6 lg:w-7 h-6 lg:h-7 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 flex items-center justify-center hover:bg-slate-200 dark:hover:bg-slate-600 text-sm font-bold">+</button>
                </div>
              </div>
            ))
      ...
      <Dialog open={isCheckoutOpen} onOpenChange={setIsCheckoutOpen}>
        <DialogContent className="max-w-sm lg:max-w-md">
          ...
          <div className="space-y-4 lg:space-y-6 py-4">
            <div className="flex justify-between items-center p-3 lg:p-4 bg-slate-50 dark:bg-slate-700 rounded-xl">
              <span className="font-headline font-bold text-base lg:text-lg text-slate-900 dark:text-slate-50">{t("total")}</span>
              <span className="font-headline font-black text-blue-700 dark:text-blue-400 text-xl lg:text-2xl">{formatIDR(subtotal)}</span>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-bold text-slate-600 dark:text-slate-300">{t("paymentMethod")}</label>
            ...
            {paymentMethod === "CASH" && (
              <div className="space-y-2">
                <label className="text-sm font-bold text-slate-600 dark:text-slate-300">{t("cashPaid")}</label>
                <input type="number" value={cashPaid} onChange={e => setCashPaid(e.target.value)} className="w-full p-3 lg:p-4 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-50 text-base lg:text-xl font-headline font-bold placeholder:text-slate-400 dark:placeholder:text-slate-500" placeholder="0.00" autoFocus />
                {cashPaid && parseFloat(cashPaid) >= subtotal && (
                  <div className="flex justify-between p-3 lg:p-4 bg-green-50 dark:bg-green-900/20 rounded-xl">
                    ...
                    <span className="font-headline font-black text-green-700 dark:text-green-400 text-base lg:text-xl">{formatIDR(change)}</span>
                  </div>
                )}
              </div>
            )}
          ...
          <DialogFooter className="gap-2">
            <button onClick={() => setIsCheckoutOpen(false)} className="flex-1 px-4 lg:px-6 py-2 lg:py-3 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl font-headline font-bold hover:bg-slate-200 dark:hover:bg-slate-600 text-sm lg:text-base">{tCommon("cancel")}</button>
      ...
      <Dialog open={isReceiptOpen} onOpenChange={setIsReceiptOpen}>
        <DialogContent className="max-w-sm">
          ...
          <DialogFooter className="flex-col gap-2">
            ...
            <button onClick={() => setIsReceiptOpen(false)} className="w-full px-4 lg:px-6 py-2 lg:py-3 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl font-headline font-bold hover:bg-slate-200 dark:hover:bg-slate-600 text-sm lg:text-base">{t("newSale")}</button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
