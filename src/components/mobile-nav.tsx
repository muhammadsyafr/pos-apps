...
export function MobileNav() {
  ...
  return (
    <>
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-canvas-light dark:bg-canvas-night-elevated border-t border-hairline-light dark:border-hairline-dark z-30 safe-area-inset-bottom">
        <div className="flex items-center justify-around px-2 py-2">
          ...
          <a
            href={`/${locale}/pos`}
            className={`flex flex-col items-center gap-1 px-6 py-2 -mt-4 rounded-full transition-all ${
              pathname === `/${locale}/pos`
                ? 'bg-ink dark:bg-on-dark text-on-primary dark:text-canvas-night shadow-lg'
                : 'bg-ink dark:bg-on-dark text-on-primary dark:text-canvas-night shadow-lg'
            }`}
          >
      ...
      {showMore && (
        <>
          ...
          <div className="lg:hidden fixed bottom-16 right-2 bg-canvas-light dark:bg-canvas-night-elevated rounded-xl shadow-xl border border-hairline-light dark:border-hairline-dark z-50 w-48">
            <div className="py-2">
              {!isCashier && (
                <a
                  href={`/${locale}/dashboard/categories`}
                  onClick={() => setShowMore(false)}
                  className="flex items-center gap-3 px-4 py-3 text-sm text-ink dark:text-on-dark hover:bg-shade-30 dark:hover:bg-white/5"
                >
